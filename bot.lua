...
chats = {}
  -----------------------------------------------------------------------------------------------
                                     -- start functions --
  -----------------------------------------------------------------------------------------------
function is_masih(msg)
  local var = false
  local masih = {326824638}
  for k,v in pairs(masih) do
...
if database:get('bot:forward:mute'..msg.chat_id_) and not is_mod(msg.sender_user_id_, msg.chat_id_) then
  ...
  setphoto(msg.chat_id_, msg.content_.photo_.sizes_[1].photo_.persistent_id_)
        send(msg.chat_id_, msg.id_, 1, '*Group photo updated!*', 1, 'md')
...
if database:get('bot:forward:mute'..msg.chat_id_) and not is_mod(msg.sender_user_id_, msg.chat_id_) then
  ...
  if database:get('bot:music:mute'..msg.chat_id_) and not is_mod(msg.sender_user_id_, msg.chat_id_) then
   ...
   if caption_text then
   if database:get('bot:links:mute'..msg.chat_id_) and not is_mod(msg.sender_user_id_, msg.chat_id_) then
	if caption_text:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Mm][Ee]") or caption_text:match("[Tt][Ll][Gg][Rr][Mm].[Mm][Ee]") or caption_text:match("[Tt][Ll].[Mm][Ee]") then
    local id = msg.id_
        local msgs = {[0] = id}
        local chat = msg.chat_id_
        delete_msg(chat,msgs)
...
if database:get('bot:forward:mute'..msg.chat_id_) and not is_mod(msg.sender_user_id_, msg.chat_id_) then
  ...
  return 
	...
	if database:sismember(hash, result.sender_user_id_) then
         ...
         send(msg.chat_id_, msg.id_, 1, '*User* `'..result.sender_user_id_..'` *promoted as moderator.*', 1, 'md')
	end
    end
	...
	if not database:sismember(hash, result.sender_user_id_) then
         send(msg.chat_id_, msg.id_, 1, '*User* `'..result.sender_user_id_..'` *is not Promoted.*', 1, 'md')
	else
         database:srem(hash, result.sender_user_id_)
         send(msg.chat_id_, msg.id_, 1, '*User* `'..result.sender_user_id_..'` *Demoted.*', 1, 'md')
	end
    end
    ...
    if database:sismember(hash, result.sender_user_id_) then
         ...
         database:sadd(hash, result.sender_user_id_)
         send(msg.chat_id_, msg.id_, 1, '*User* `'..result.sender_user_id_..'` *Muted.*', 1, 'md')
	end
    end
	end
	...
	send(msg.chat_id_, msg.id_, 1, '*User* `'..ap[2]..'` *Muted.*', 1, 'md')
	end
    end
	-----------------------------------------------------------------------------------------------
	...
	local hash = 'bot:muted:'..msg.chat_id_
	if not database:sismember(hash, result.sender_user_id_) then
         send(msg.chat_id_, msg.id_, 1, '*User* `'..result.sender_user_id_..'` *is not Muted.*', 1, 'md')
	else
         database:srem(hash, result.sender_user_id_)
         send(msg.chat_id_, msg.id_, 1, '*User* `'..result.sender_user_id_..'` *Unmuted.*', 1, 'md')
	end
    end
	...
	send(msg.chat_id_, msg.id_, 1, '*User* `'..ap[2]..'` *Unmuted.*', 1, 'md')
    end
	-----------------------------------------------------------------------------------------------
	...
	if database:sismember(hash, result.sender_user_id_) then
         ...
         database:sadd(hash, result.sender_user_id_)
         send(msg.chat_id_, msg.id_, 1, '*User* `'..result.sender_user_id_..'` *Promoted as Group Owner.*', 1, 'md')
	end
    end
	...
	send(msg.chat_id_, msg.id_, 1, '*User* `'..ap[2]..'` *Promoted as Group Owner.*', 1, 'md')
    end
	-----------------------------------------------------------------------------------------------
	if text:match("^[#!/]demowner$") and is_masih(msg.sender_user_id_) and msg.reply_to_message_id_ then
	function deowner_by_reply(extra, result, success)
	local hash = 'bot:owners:'..msg.chat_id_
	if not database:sismember(hash, result.sender_user_id_) then
         send(msg.chat_id_, msg.id_, 1, '*User* `'..result.sender_user_id_..'` *is not Owner.*', 1, 'md')
	else
         database:srem(hash, result.sender_user_id_)
         send(msg.chat_id_, msg.id_, 1, '*User* `'..result.sender_user_id_..'` *Removed from ownerlist.*', 1, 'md')
	end
    end
	...
	local ap = {string.match(text, "^[#/!](demowner) (%d+)$")} 	
         database:srem(hash, ap[2])
	send(msg.chat_id_, msg.id_, 1, '*User* `'..ap[2]..'` *Removed from ownerlist.*', 1, 'md')
    end
	-----------------------------------------------------------------------------------------------
	if text:match("^[#!/]addadmin$") and is_masih(msg) and msg.reply_to_message_id_ then
	...
	-----------------------------------------------------------------------------------------------
	if text:match("^[#!/]addadmin @(.*)$") and is_masih(msg) then
	...
	-----------------------------------------------------------------------------------------------
	if text:match("^[#!/]addadmin (%d+)$") and is_masih(msg) then
	...
	-----------------------------------------------------------------------------------------------
	if text:match("^[#!/]remadmin$") and is_masih(msg) and msg.reply_to_message_id_ then
	...
	-----------------------------------------------------------------------------------------------
	if text:match("^[#!/]remadmin @(.*)$") and is_masih(msg) then
	local hash = 'bot:adminss:'
	...
	-----------------------------------------------------------------------------------------------
	if text:match("^[#!/]remadmin (%d+)$") and is_masih(msg) then
	local hash = 'bot:adminss:'
	...
	-----------------------------------------------------------------------------------------------
	if text:match("^[#!/]owner$") or text:match("^[#!/]ownerlist$") and is_masih(msg) then
    local hash =  'bot:owners:'..msg.chat_id_
	local list = database:smembers(hash)
	...
	-----------------------------------------------------------------------------------------------
	if text:match("^[#!/]adminlist$") and is_masih(msg) then
    local hash =  'bot:adminss:'
	local list = database:smembers(hash)
  ...
  if text:match("^[#!/]kick$") and msg.reply_to_message_id_ and is_mod(msg.sender_user_id_, msg.chat_id_) then
   ...
   getMessage(msg.chat_id_,msg.reply_to_message_id_,kick_reply)
    ...
    -----------------------------------------------------------------------------------------------
  if text:match("^[#!/]inv$") and msg.reply_to_message_id_ and is_mod(msg) then
...
--vardump(result)
   ...
   tdcli_function ({
	...
	if tonumber(floodmax[2]) < 2 then
         send(msg.chat_id_, msg.id_, 1, '*Wrong number*,*range is* `[2-99999]`', 1, 'md')
	else
    database:set('flood:max:'..msg.chat_id_,floodmax[2])
         send(msg.chat_id_, msg.id_, 1, '*> Flood has been set to* `'..floodmax[2]..'`', 1, 'md')
	end
	end
	-----------------------------------------------------------------------------------------------
    ...
    database:set('flood:time:'..msg.chat_id_,floodt[2])
         send(msg.chat_id_, msg.id_, 1, '*> Flood Time has been set to* `'..floodt[2]..'`', 1, 'md')
	end
	end
	-----------------------------------------------------------------------------------------------
	...
	local link = database:get('bot:link'..msg.chat_id_)
         send(msg.chat_id_, msg.id_, 1, link, 1, nil)
    end
	-----------------------------------------------------------------------------------------------
	...
	local unmutept = {string.match(text, "^[#/!](unmute) (.*)$")} 
      if unmutept[2] == "all" then
         send(msg.chat_id_, msg.id_, 1, '`Mute All` *has been Disabled*', 1, 'md')
         database:del('bot:muteall'..msg.chat_id_)
      end
	  if unmutept[2] == "text" then
         send(msg.chat_id_, msg.id_, 1, '`Text` *Has been Unmuted*', 1, 'md')
         database:del('bot:text:mute'..msg.chat_id_)
      end
	  if unmutept[2] == "photo" then
         send(msg.chat_id_, msg.id_, 1, '`Photo` *Has been Unmuted*', 1, 'md')
         database:del('bot:photo:mute'..msg.chat_id_)
      end
	  if unmutept[2] == "video" then
         send(msg.chat_id_, msg.id_, 1, '`Video` *Has been Unmuted*', 1, 'md')
         database:del('bot:video:mute'..msg.chat_id_)
      end
	  if unmutept[2] == "inline" then
         send(msg.chat_id_, msg.id_, 1, '`inline` *Has been Unmuted*', 1, 'md')
         database:del('bot:inline:mute'..msg.chat_id_)
      end
	  if unmutept[2] == "gifs" then
         send(msg.chat_id_, msg.id_, 1, '`Gifs` *Has been Unmuted*', 1, 'md')
         database:del('bot:gifs:mute'..msg.chat_id_)
      end
	  if unmutept[2] == "music" then
         send(msg.chat_id_, msg.id_, 1, '`Music` *Has been Unmuted*', 1, 'md')
         database:del('bot:music:mute'..msg.chat_id_)
      end
	  if unmutept[2] == "voice" then
         send(msg.chat_id_, msg.id_, 1, '`Voice` *Has been Unmuted*', 1, 'md')
	...
	local ex = database:ttl("bot:charge:"..msg.chat_id_)
 	...
 	------------
	local TXT = "*#Group_Settings:*\n\n"

  .."*>#Welcome_Msg* = `"..send_welcome.."`\n"
  .."*>Mute #Stickers* = `"..lock_sticker.."`\n"
  .."*>Mute #TgServices* = `"..lock_tgservice.."`\n"
  .."*>Mute #Links* = `"..mute_links.."`\n"
  .."*>Mute #WebPage* = `"..lock_wp.."`\n"
  .."*>Mute #Tag{@}/Mention* = `"..lock_tag.."`\n"
  .."*>Mute #Hashtag{#}* = `"..lock_htag.."`\n" 
  .."*>Mute #Contacts* = `"..lock_contact.."`\n"
  .."*>Mute #English* = `"..lock_english.."`\n"
  .."*>Mute #Location* = `"..lock_location.."`\n"
  .."*>Lock #Bots* = `"..mute_bots.."`\n"
  .."*>Mute #Inline* = `"..mute_in.."`\n"
  .."*>Mute #Arabic / #Persian* = `"..lock_arabic.."`\n"
  .."*>Mute #Forward* = `"..lock_forward.."`\n"
  .."*>Mute #Edit_Msg* = `"..mute_edit.."`\n"
  .."*>Lock #Pin_Msg* = `"..lock_pin.."`\n"
  ...
  .."*•Mute #Gifs* = `"..mute_gifs.."`\n"
  .."*•Mute #Musics* = `"..mute_music.."`\n"
  .."*•Mute #Voice_Msgs* = `"..mute_voice.."`\n"
  ...
  -----------------------------------------------------------------------------------------------
	...
	local txt = {string.match(text, "^[#/!](rename) (.*)$")} 
         ...
         send(msg.chat_id_, msg.id_, 1, '*Group name updated!*', 1, 'md')
    end
	-----------------------------------------------------------------------------------------------
	...
	-----------------------------------------------------------------------------------------------
	if text:match("^[#!/]reload$") and is_admin(msg) then
...
*/muteuser* `[id, Username, Reply]`
`اضافه کردن کاربر به لیست افراد میوت شده`
*/unmuteuser* `[id, Username, Reply]`
  ...
  -- vardump(msg)
   ...
   if database:get('bot:english:mute'..result.chat_id_) and not is_mod(result.sender_user_id_, result.chat_id_) then
	...
	if database:get('bot:editid'..msg.message_id_) then
	    ...
	    send(msg.chat_id_, msg.message_id_, 1, '*چرا ادیت میکنی😠\nمن دیدم که گفتی:*\n\n*'..old_text..'*', 1, 'md')
	end
	end
